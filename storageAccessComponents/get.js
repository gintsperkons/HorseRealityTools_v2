...
export var get = {
    handleDataRequests: function(request, sendResponse, horseData, horseConfig, currentHorseId) {
        if (request["type"] === "name") {
            names.handleDataRequests(request, sendResponse);
        } else if (request["type"] === "horse") {
            horses.handleDataRequests(request, sendResponse, horseData, currentHorseId);
