    ...
    setListeners: function() {
        ...
        for (const but of document.getElementsByClassName("removeTemplateButton")) {
            but.addEventListener("click", (event) => {
                ...
                var el = document.getElementById(but.getAttribute("templateName") + "List")
...
function inputTextChange(event) {
    storage.getHorseConfig(function(horseConfig) {
        ...
        if (!horseConfig["templates"][event.target.id]) horseConfig["templates"][event.target.id] = {};
        ...
        if (document.getElementById(event.target.id + "List").value == "addNew") {
            ...
            while (true) {
                if (!horseConfig["templates"][event.target.id][count]) {
                    if (!document.getElementById(event.target.id + "List").querySelector("option[value='" + count + "']")) {
                        var el = document.createElement("option");
                        el.value = count;
        ...
        var listItem = document.getElementById(event.target.id + "List").querySelector("option[value='" + count + "']");
...
function selectElementChangeValue(event) {
    storage.getHorseConfig(function(horseConfig) {
        ...
        var dataElement = event.target.querySelector("option[value='" + event.target.value + "']");
...
function removeTemplate(list, value) {
    ...
    console.log(value);
    storage.getHorseConfig(function(horseConfig) {
        console.log(horseConfig["templates"]);
        console.log(horseConfig["templates"][list][value]);
        if ((!horseConfig["templates"]) || (!horseConfig["templates"][list])) {
            return;
        }
        document.getElementById(list + "List").querySelector("option[value='" + value + "']").remove()
