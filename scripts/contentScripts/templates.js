...
const achevementCountText = (template, base, num) => {
  ...
  return template.replace(
    base,
    base.replace("{", "").replace("}", "").toUpperCase() + num
  );
};
...
const templateToText = (template) => {
  ...
  try {
    ...
    cobt = "";
    if (
      horseData["training"]["currentTraining"].trim() ===
      "Basic Training".trim()
    ) {
  ...
  try {
    ...
    template = template.replaceAll( "{vg}", vg);
    template = template.replaceAll( "{g}", g);
    template = template.replaceAll( "{a}", a);
    template = template.replaceAll( "{ba}", ba);
    template = template.replaceAll( "{p}", p);
    template = template
      .replace("  ", " ")
      .replace("   ", " ")
      .replace("    ", " ")
...
const addUpdateData = (conf) => {
  conf = conf["data"]["horseToolConfig"];
  if (!conf) {
    return;
  }
  ...
  if (!nameRowChildListNames.includes("select")) {
    ...
    nameList.addEventListener("change", (r) => {
      val = document
        .querySelector("option[value='" + r.target.value + "']")
        .textContent.trim();
...
function getUpdateTab() {
  ...
  document
    .getElementById("tab_update")
    .addEventListener("click", addUpdateDataWhenReady, { once: true });
}
